...
async function up(knex) {
    ...
    try {
        await knex.raw("ALTER TABLE \"schedules\" ADD CONSTRAINT \"schedules_type_check\" CHECK (type IN ('medicine','feeding','milk','nap','diaper','bath','play','other'))");
    }
    catch (e) {
...
async function down(knex) {
    ...
    try {
        await knex.raw('ALTER TABLE "schedules" DROP CONSTRAINT IF EXISTS "schedules_type_check"');
        await knex.raw("ALTER TABLE \"schedules\" ADD CONSTRAINT \"schedules_type_check\" CHECK (type IN ('medicine','feeding','milk'))");
    }
    catch (e) {
        // ignore
    }
