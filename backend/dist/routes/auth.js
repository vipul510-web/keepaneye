...
const validateRegistration = [
    ...
    (0, express_validator_1.body)('role').isIn(['parent', 'caregiver']).withMessage('Invalid role')
];
const validateLogin = [
    (0, express_validator_1.body)('email').isEmail().normalizeEmail(),
...
router.post('/register', validateRegistration, async (req, res) => {
    try {
        ...
        // Create user
        const result = await (0, database_1.db)('users').insert({
            email,
        ...
        // Generate JWT token
        // @ts-ignore
        const token = jsonwebtoken_1.default.sign({ userId, email, role }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '24h' });
...
router.post('/login', validateLogin, async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '24h' });
...
router.post('/caregiver-setup', [
    ...
    try {
        ...
        const { email, password } = req.body;
        // Find user by email
        const user = await (0, database_1.db)('users').where({ email, role: 'caregiver' }).first();
        if (!user) {
