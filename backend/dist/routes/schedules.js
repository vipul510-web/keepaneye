...
router.get('/', async (req, res) => {
    try {
        ...
        const schedules = await (0, database_1.db)('schedules')
...
router.post('/generate', async (req, res) => {
    try {
        ...
        const allTemplateIds = await (0, database_1.db)('schedule_templates')
            .where({ child_id: childId })
            .select('id');
        const results = [];
        for (const dateStr of dates) {
            ...
            const deletedSchedules = await (0, database_1.db)('schedules')
                .where({ child_id: childId })
                .andWhere('scheduled_time', '>=', start)
                .andWhere('scheduled_time', '<', end)
            ...
            for (const tpl of templates) {
                ...
                if (tpl.frequency === 'daily')
                    ...
                    shouldGenerate = tpl.weekday === weekday;
                else if (tpl.frequency === 'monthly')
                    shouldGenerate = true; // optionally match day-of-month later
                ...
                const [hours, minutes, seconds] = tpl.time_of_day.split(':').map((s) => parseInt(s, 10));
                ...
                const existing = await (0, database_1.db)('schedules')
                ...
                const [createdResult] = await (0, database_1.db)('schedules').insert({
                    child_id: childId,
                    template_id: tpl.id,
                    type: tpl.type,
                    title: tpl.title,
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const schedule = await (0, database_1.db)('schedules').where({ id: scheduleId }).first();
        if (!schedule)
        ...
        if (payload.scheduled_time !== undefined)
            updates.scheduled_time = new Date(payload.scheduled_time);
        ...
        const [updated] = await (0, database_1.db)('schedules').where({ id: scheduleId }).update(updates).returning('*');
...
router.post('/replace', async (req, res) => {
    try {
        const user = req.user;
        const { childId, plan, startDate, weeks } = req.body;
        ...
        const deleted = await (0, database_1.db)('schedules')
        ...
        while (cursor < end) {
            ...
            for (const item of plan) {
                if (!item.weekdays?.includes(weekday))
                    continue;
                const [h, m, s] = (item.timeOfDay || '00:00:00').split(':').map((v) => parseInt(v, 10));
        ...
        if (toInsert.length > 0) {
            ...
            for (let i = 0; i < toInsert.length; i += chunks) {
                ...
                await (0, database_1.db)('schedules').insert(batch);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const schedule = await (0, database_1.db)('schedules').where({ id }).first();
        if (!schedule)
            return res.status(404).json({ error: 'Schedule not found' });
        // Authorization: parent must own the child, caregiver must be assigned to the child
        const canParent = await (0, database_1.db)('children').where({ id: schedule.child_id, parent_id: user.userId }).first();
        let canCaregiver = false;
        if (!canParent) {
            const rel = await (0, database_1.db)('child_caregivers').where({ child_id: schedule.child_id, caregiver_id: user.userId }).first();
            canCaregiver = !!rel;
        }
        if (!canParent && !canCaregiver)
            return res.status(403).json({ error: 'Forbidden' });
        await (0, database_1.db)('schedules').where({ id }).del();
