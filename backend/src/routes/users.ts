...
import { requireParent, requireAuth } from '../middleware/auth';
import { asyncHandler } from '../middleware/errorHandler';

const router = express.Router();

// Get all caregivers for a parent
router.get('/caregivers', requireAuth, asyncHandler(async (req, res) => {
  ...
  const caregivers = await db('child_caregivers as cc')
...
router.get('/caregivers/:childId', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (child.parent_id !== userId) {
    const caregiverAccess = await db('child_caregivers')
...
router.post('/caregivers/:childId', requireParent, asyncHandler(async (req, res) => {
  const { childId } = req.params;
  ...
  const caregiver = await db('users')
  ...
  const existingRelationship = await db('child_caregivers')
  ...
  await db('child_caregivers').insert({
    child_id: childId,
...
router.delete('/caregivers/:childId/:caregiverId', requireParent, asyncHandler(async (req, res) => {
  ...
  const deletedCount = await db('child_caregivers')
...
router.get('/children', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (userRole === 'parent') {
    ...
    children = await db('child_caregivers as cc')
      .join('children as c', 'cc.child_id', 'c.id')
      .select('c.*')
...
router.get('/stats', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (userRole === 'parent') {
    // Count children
    const childrenCount = await db('children')
      .where({ parent_id: userId })
      .count('* as count')
      .first();

    // Count caregivers
    const caregiversCount = await db('child_caregivers as cc')
      .join('children as c', 'cc.child_id', 'c.id')
      .where({ 'c.parent_id': userId })
      .count('cc.caregiver_id as count')
      .first();

    stats.childrenCount = parseInt(childrenCount?.count as string || '0');
    stats.caregiversCount = parseInt(caregiversCount?.count as string || '0');
  } else {
    // Count children under care
    const childrenCount = await db('child_caregivers')
  ...
  let schedulesQuery = db('schedules as s');
  ...
  stats.todaySchedulesCount = parseInt(todaySchedules?.count as string || '0');
